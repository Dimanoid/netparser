 ...
 *
 * @example
 * const matcher = new netparser.Matcher(['192.168.0.0/24', '192.168.4.0/23']);
...
export class Matcher {
    ...
    public constructor(networks?: string[]) {
        ...
        if (networks) {
            for (const s of networks) {
                const net = shared.parseBaseNetwork(s, false, false);
     ...
     *
     * @example
     * const matcher = new netparser.Matcher();
    ...
    public has(network: string) {
        const net = shared.parseBaseNetwork(network, false, false);

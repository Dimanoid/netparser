import { GenericFindLongestZeroHextetChain, GenericBytesToAddr } from './errors';
...
function findLongestZeroHextetChain(bytes: number[], throwErrors?: boolean) {
    ...
    if (throwErrors) throw GenericFindLongestZeroHextetChain;
    return null;
}
...
export function bytesToAddr(bytes: number[], throwErrors?: boolean) {
    ...
    if (throwErrors) throw GenericBytesToAddr;
    return null;
}

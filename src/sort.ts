...
export function isSorted(networks: Network[]) {
	if (networks.length > 1) {
		for (let i = 1; i < networks.length; i++) {
			const x = networks[i - 1].compare(networks[i]);
...
export function insertionSort(networks: Network[]) {
	...
	networks.forEach((net: Network) => {
		const idx = binarySearchForInsertionIndex(net, sorted);
...
function msdRadixSort(networks: Network[], start: number, stop: number, byteIndex: number) {
	...
	if (byteIndex < 16) {
		let lastPrefixSum = 0;
		for (const rps of runningPrefixSum) {
			if (rps !== lastPrefixSum) {
				msdRadixSort(networks, start + lastPrefixSum, start + rps, byteIndex + 1);
			}
			lastPrefixSum = rps;
		}
	}

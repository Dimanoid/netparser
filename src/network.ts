...
import { InvalidSubnet, NotValidCIDR } from './errors';
...
export class Network {
    ...
    private netbits: number = -1;

    public constructor(network?: string | Network, throwErrors?: boolean) {
        if (network) {
            if (network instanceof Network) {
                this.addr.setBytes(network.addr.bytes().slice());
                this.netbits = network.netbits;
            }
            else {
                const net = parse.network(network, throwErrors);
                if (net) {
    ...
    public previous(): Network {
        this.addr.decrease(this.netbits);
        return this;
    }

    public lastAddr(): Address {
    ...
    public setCIDR(cidr: number, throwErrors?: boolean): Network {
        if (!this.addr.isValid()) {
            if (throwErrors) throw InvalidSubnet;
            this.destroy();
        } else {
            ...
            if (cidr >= 0 && cidr <= this.addr.bytes().length * 8) {
                ...
                if (throwErrors) throw NotValidCIDR;
                this.destroy();
            }
        }
        return this;
    ...
    public contains(network: Network): boolean {
        // check that both networks are valid

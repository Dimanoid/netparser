import { GenericNetworkParse } from './errors';
...
export function network(s: string, throwErrors?: boolean) {
    ...
    if (isIPv4 === null) {
        if (throwErrors) throw GenericNetworkParse;
        return null;
    }
...
function parseIntRange(old: string, min: number, max: number) {
    let s = '';
    for (const o of old) {
        if (Number.isNaN(parseInt(o, 10))) break;
        s += o;
    }
...
function parseLeftHalf(bytes: number[], leftHalf: string) {
    ...
    if (leftHalf !== '') {
        ...
        for (const lp of leftParts) {
            ...
            const ipv4Parts = lp.split('.');
            if (ipv4Parts.length === 0) return null;
            if (ipv4Parts.length !== 4) {
                const x = parseHextet(lp);

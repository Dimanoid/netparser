 ...
 */
export function baseAddress(networkAddress: string | Network, throwErrors?: boolean): Address | null {
    const net = networkAddress instanceof Network ? networkAddress : new Network(networkAddress, throwErrors);
 ...
 *
 * @param net - A network like 192.168.0.0/24
 * @param throwErrors - Stop the library from failing silently
 *
 * @returns An IPv4 address or null in case of error
 */
export function broadcastAddress(net: string | Network, throwErrors?: boolean): Address | null {
    const _net = new Network(net, throwErrors);
    if (!_net.isValid() || _net.addr.bytes().length === 16) return null;
    _net.addr.applySubnetMask(_net.cidr());
    return _net.lastAddr();
}
 ...
 */
export function findUnusedSubnets(aggregate: string, subnets: string[], strict?: boolean, throwErrors?: boolean): Network[] | null {
    ...
    return shared.findNetworkGaps(agg, subnetworks);
}
 ...
 */
export function ip(address: string, throwErrors?: boolean): string | null {
 ...
 */
export function network(networkAddress: string, throwErrors?: boolean): string | null {
 ...
 * @returns A boolean or null in case of error
 */
export function networkComesBefore(net: string, otherNetwork: string, strict?: boolean, throwErrors?: boolean): boolean | null {
    const alphaNet = shared.parseBaseNetwork(net, strict, throwErrors);
 ...
 * @returns A boolean or null in case of error
 */
export function networkContainsAddress(net: string, address: string, strict?: boolean, throwErrors?: boolean): boolean | null {
    const _net = shared.parseBaseNetwork(net, strict, throwErrors);
    if (!_net || !_net.isValid()) return null;
    const addrNet = new Network(address, throwErrors);
    if (!addrNet.isValid()) return null;
    if (addrNet.cidr() !== addrNet.addr.bytes().length * 8) {
        if (throwErrors) throw InvalidAddress;
        return null;
    }
    return _net.contains(addrNet);
}
 ...
 *
 * @param net - A network like 192.168.0.0/16
 ...
 * @returns A boolean or null in case of error
 */
export function networkContainsSubnet(net: string, subnet: string, strict?: boolean, throwErrors?: boolean): boolean | null {
    const alphaNet = shared.parseBaseNetwork(net, strict, throwErrors);
 ...
 *
 * @param net - A network like 192.168.0.0/23
 * @param otherNetwork - A network like 192.168.1.0/24
 * @param strict - Do not automatically mask addresses to baseAddresses
 * @param throwErrors - Stop the library from failing silently
 *
 * @returns A boolean or null in case of error
 */
export function networksIntersect(net: string, otherNetwork: string, strict?: boolean, throwErrors?: boolean): boolean | null {
    const alphaNet = shared.parseBaseNetwork(net, strict, throwErrors);
    if (!alphaNet) return null;
    const bravoNet = shared.parseBaseNetwork(otherNetwork, strict, throwErrors);
    if (!bravoNet) return null;
 ...
 */
export function nextAddress(address: string, throwErrors?: boolean): string | null {
    const addr = new Address(address, throwErrors);
    if (!addr.isValid()) return null;
    if (!addr.next().isValid()) {
        if (throwErrors) throw OverflowedAddressSpace;
        return null;
    }
 ...
 */
export function nextNetwork(net: string, strict?: boolean, throwErrors?: boolean): string | null {
    const _net = shared.parseBaseNetwork(net, strict, throwErrors);
    if (!_net || !_net.isValid()) return null;
    if (!_net.next().isValid()) {
        if (throwErrors) throw OverflowedAddressSpace;
        return null;
    }
 ...
 */
export function rangeOfNetworks(startAddress: string, stopAddress: string, throwErrors?: boolean): string[] | null {
    ...
    if (startAddr.bytes().length !== stopAddr.bytes().length) {
        if (throwErrors) throw MixingIPv4AndIPv6;
        return null;
    }
 ...
 * @returns An array of networks or null in case of error
 */
export function sort(networkAddresses: string[], throwErrors?: boolean): string[] | null {
 ...
 * @returns An array of networks or null in case of error
 */
export function summarize(networks: string[], strict?: boolean, throwErrors?: boolean): string[] | null {
    let subnets = [] as Network[];
    for (const n of networks) {
        const net = shared.parseBaseNetwork(n, strict, false);
        if (net) {
            if (net.isValid()) {
                subnets.push(net);
            }
            ...
            if (throwErrors) throw NotValidBaseNetworkAddress;
            return null;
        }
    }
    shared.sortNetworks(subnets);

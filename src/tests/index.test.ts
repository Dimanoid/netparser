...
test('sanity check baseAddress #1', () => {
    const output = index.baseAddress('192.168.200.113/24', true).toString();
    expect(output).toEqual('192.168.200.0');
});

test('sanity check baseAddress #2', () => {
    const output = index.baseAddress('ffff:fc00::1:1234/64', true).toString();
...
test('sanity check broadcastAddress #1', () => {
    const output = index.broadcastAddress('192.168.0.0/24', true).toString();
...
test('sanity check findUnusedSubnets #1', () => {
    const output = index.findUnusedSubnets(
        '192.168.0.0/22',
        ['192.168.2.32/30', '192.168.1.0/24'],
        true,
        true
    ).map(n => n.toString());
...
test('sanity check findUnusedSubnets #2', () => {
    const output = index.findUnusedSubnets('192.168.0.0/22', []).map(n => n.toString());
...
test('sanity check findUnusedSubnets #4', () => {
    const output = index.findUnusedSubnets('192.168.0.0/24', ['192.168.0.0/25', '192.168.0.128/26'], true, true).map(n => n.toString());
    expect(output).toEqual(['192.168.0.192/26']);
});

test('sanity check findUnusedSubnets #5', () => {
    const output = index.findUnusedSubnets('0.0.0.0/0', ['192.168.0.0/25', '192.168.0.128/26'], true, true).map(n => n.toString());
    expect(output).toEqual([
        '0.0.0.0/1',
        '128.0.0.0/2',
        '192.0.0.0/9',
